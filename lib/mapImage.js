...
function storeTileCache()
{
	if (!tileCacheStorage) return;

	const entries = [...tileCache].map(([key, entry]) => ({
		key,
...
function initializeTileCache(storage)
{
	...
	const entries = storage.get('mapTileCache');
	if (!Array.isArray(entries)) return;

	entries.forEach((entry) =>
	{
		if (!entry || typeof entry.key !== 'string' || typeof entry.buffer !== 'string' || !Number.isFinite(entry.expires)) return;
		try
		{
			tileCache.set(entry.key, {
		...
		{
			// Ignore individual corrupt entries and keep the rest of the cache usable.
		}
	});
...
{
	return { ...tileStats, cacheSize: tileCache.size };
}
