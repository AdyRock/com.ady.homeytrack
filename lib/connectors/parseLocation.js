...
function parseLocationPayload(raw, context = {})
{
	...
	if (!Number.isFinite(raw.lat) || raw.lat < -90 || raw.lat > 90
		|| !Number.isFinite(raw.lon) || raw.lon < -180 || raw.lon > 180
		|| !Number.isFinite(raw.tst) || raw.tst <= 0)
	{
		return null;
	}
...
/**
 * Parses waypoint reports while preserving whether the phone sent one waypoint or a complete export.
 * @param {object} raw The parsed JSON payload sent by OwnTracks.
 * @param {{ user?: string, device?: string }} [context] Fallback source identity.
 * @returns {{ complete: boolean, waypoints: object[], user: string, device: string, topic: string|null }|null}
 */
function parseWaypointReport(raw, context = {})
{
	if (!raw || (raw._type !== 'waypoint' && raw._type !== 'waypoints'))
	{
		return null;
	}
	...
	return {
		complete: raw._type === 'waypoints',
		waypoints: parseWaypointPayloads(raw),
		user: context.user || (topic ? topicToUser(topic) : 'unknown'),
		device: context.device || (topic ? topicToDevice(topic) : 'unknown'),
		topic,
	};
}
...
module.exports = { parseLocationPayload, parseWaypointPayloads, parseWaypointReport };
